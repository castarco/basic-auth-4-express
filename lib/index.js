...
function buildMiddleware(options) {
    ...
    const isAsync = options.authorizeAsync !== undefined
        ? !!options.authorizeAsync
        : false;
    const passRequest = options.passRequest !== undefined
        ? !!options.passRequest
        : false;
    ...
    return function authMiddleware(req, res, next) {
        ...
        if (isAsync) {
            return passRequest
                ? authorizer(req, authentication.name, authentication.pass, authorizerCallback)
                : authorizer(authentication.name, authentication.pass, authorizerCallback);
        }
        if (passRequest) {
            if (!authorizer(req, authentication.name, authentication.pass)) {
                return unauthorized();
            }
        } else {
